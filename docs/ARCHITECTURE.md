...
별도의 젯슨나노가 담당하는 역할 분담 구조였다.

- **Raspberry Pi** (+ 카메라, + ReSpeaker 2 Mics Pi HAT) — 3가지 역할을 겸했다.
  1. 카메라로 영상을 캡처
...
- **Jetson Nano** — 라즈베리파이가 송출한 스트림을 `cv2.VideoCapture()`로 받아와 YOLOv5로
  재료/손 탐지만 전담. 카메라나 motionEye를 직접 실행하지는 않는다.
...
`jetson/config.py`의 `CAMERA_STREAM_URL`은 실제로는 라즈베리파이의 IP를 가리켜야 한다.

## 검증된(실제 동작이 확인되는) 흐름

- `jetson/ingredient_locator_server.py` (원본 JetDo.py) — 소켓 **서버**. 재료명을 받아
  YOLOv5 탐지 후 "재료 있음/없음" + 시 방향을 응답.
- `pi/ingredient_locator_client.py` (원본 PiDo.py) — 소켓 **클라이언트**. Google STT로
  재료명을 인식해 서버에 전송, 응답을 받아 안내 음성 재생.

두 파일은 host/port, 요청·응답 프로토콜이 정확히 맞물리는 유일한 조합이라, 실제로
전원을 켜면 자동 실행되던 조합이었을 가능성이 가장 높다. (단, 사용자 본인 확인으로도
"기능을 완성시키지는 못했다"고 하여, 끝까지 안정적으로 통합·디버깅되었는지는 불확실하다.)

## 미완성으로 남은 흐름

`experiments/wake_word_assistant.py` (원본 end/pi_1.py)는 "웨이크워드 → STT 명령 인식
→ TTS 응답"까지 파이 쪽에서 전부 처리하려던 더 발전된 버전으로 보인다. 하지만 이 파일은
파이 쪽도 소켓 **서버**로 열어놓기 때문에, 역시 서버인 `ingredient_locator_server.py`와는
애초에 연결될 수 없는 구조다. 즉 두 컴포넌트 모두 "연결을 기다리기만" 하고 있어 실제로는
서로 대화가 불가능했다. `handle_command()` 함수도 "where is" 명령을 인식만 하고 실제
동작은 비어 있는 TODO 상태였다.
...
자동 실행 파이프라인으로 완전히 통합하는 마지막 단계는 마치지 못한 상태로 남아 있다.
자세한 근거는 [KNOWN_ISSUES.md](./KNOWN_ISSUES.md)를 참고.
