...
인식하고, 사용자가 음성으로 "빵 어디 있어?"라고 물으면 손을 기준으로 몇 시 방향에 있는지
...
![젯슨나노에서 실시간으로 동작 중인 YOLOv5 탐지 화면](./assets/photos/live-detection.png)

## 이 저장소에 대해

원본 프로젝트는 Jetson Nano + Raspberry Pi 두 대의 임베디드 보드에서 동작했는데, 두 보드
모두 하드웨어 오류가 발생하면서 실제 구동 환경과 최신 실행 이력을 잃어버렸습니다.
개발 과정에서 남겨둔 소스 코드 사본이 남아있어, 그것을 다시 분석해서 정리했습니다.
