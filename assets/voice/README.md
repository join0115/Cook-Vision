...
`pi/ingredient_locator_client.py` 등에서 재생하는 안내 음성(mp3/wav) 모음입니다.
...
공백이 붙은 중복 폴더가 별도로 있었습니다 (`_clock/`, `_ingredient/`, `_step_1/`~`_step_4/`,
예: `"1 .mp3"`처럼 숫자와 확장자 사이에 공백). OS별 파일 복사 과정에서 생긴 것으로 보이는
사실상 동일한 사본이라 포트폴리오 정리본에는 포함하지 않았습니다.
...
**단수형 폴더명 + 소문자 파일명**입니다. 또한 `step_4/invalid_ingredient.wav`도 코드에는
참조되지만 실제로는 존재하지 않습니다. 프로젝트가 끝까지 통합 테스트되지 못했음을 보여주는
증거라서 임의로 파일명을 바꾸지 않고 그대로 두었습니다.
